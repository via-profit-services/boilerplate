...
class ClientsService implements ClientsServiceinterface {
  ...
  public async getClientsConnection(
    ...
    const { first, after, last, before, id, orderBy, search, status, legalStatus } = props;
    ...
    if (cursor.orderBy) {
      cursor.orderBy.forEach(({ field, direction }) => {
        knexOrderBy.orderBy(`c.${field}`, direction);
      });
    }

    const request = this.#knex
      .select(['c.*', this.#knex.raw('json_agg(distinct "p"."id") as "persons"')])
      .groupBy(['c.id'])
      .limit(limit)
      .offset(offset)
      .leftJoin({ p: 'persons' }, 'p.client', 'c.id')
      .from<
        ClientsTableModel,
        ReadonlyArray<
          ClientsTableRecord & {
    ...
    if (cursor.orderBy) {
      cursor.orderBy.forEach(({ field, direction }) => {
        request.orderBy(`c.${field}`, direction);
      });
      const orderRaw = knexOrderBy.toQuery().replace('select *', '').trim();
      request.select([
        this.#knex.raw(`lag(??) over(${orderRaw}) as "prev"`, ['c.id']),
        this.#knex.raw(`lead(??) over(${orderRaw}) as "next"`, ['c.id']),
      ]);
    } else {
      request.select([
        this.#knex.raw(`lag(??) over() as "prev"`, ['c.id']),
        this.#knex.raw(`lead(??) over() as "next"`, ['c.id']),
      ]);
    }

    if (id) {
      request.whereIn('c.id', id);
    }

    if (status) {
      request.whereIn('c.status', status);
    }

    if (legalStatus) {
      request.whereIn('c.legalStatus', legalStatus);
    }

    if (search) {
      request.where(builder => {
        search.forEach(({ field, query }) => {
          builder.orWhereRaw('??::text ilike ?', [`c.${field}`, `%${query}%`]);
        });
      });
    }
