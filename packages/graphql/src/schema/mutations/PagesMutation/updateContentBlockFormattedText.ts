...
const updateContentBlockFormattedText: GraphQLFieldConfig<unknown, Context, Args> = {
  type: new GraphQLNonNull(ContentBlockMutationPayload),
  args: {
    id: { type: new GraphQLNonNull(GraphQLID) },
    lexical: { type: new GraphQLNonNull(GraphQLString) },
    name: { type: GraphQLString },
    page: { type: GraphQLString },
    template: { type: GraphQLString },
  },
  resolve: async (_parent, args, context) => {
    const { id, lexical, name, page, template } = args;
    ...
    if (cb.type !== 'FORMATTED_TEXT') {
      return {
        ...
        msg: 'This content block are not a formatted text type',
      };
    }

    try {
      await services.webpages.updateContentBlockFormattedText({
        id,
        name,
        page,
        template,
        updatedAt,
        lexical: typeof lexical !== 'undefined' ? JSON.stringify(lexical) : undefined,
      });
    } catch (err) {
...
export default updateContentBlockFormattedText;
