...
const selector = createStructuredSelector({
  ...
  accessToken: (store: ReduxStore) => store.auth.accessToken?.token || null,
});
...
const RelayProvider: React.FC<RelayProviderProps> = props => {
  ...
  if (accessTokenRef.current !== accessToken) {
    ...
    if (accessToken && isTokenNotExpired(accessToken)) {
      ...
      // Force to restart socket
      console.debug('[Relay] Subscription force restart to update connectionParams');
      subscriptionSocketRef.current?.close(4205, 'Client Restart');
    }
  }
